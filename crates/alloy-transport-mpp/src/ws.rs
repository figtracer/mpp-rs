...
impl<P> MppWsConnect<P, NoVoucher> {
    ...
    pub fn new<S: Into<String>>(url: S, payment_provider: P) -> Self {
        let url = url.into();
        let auth = Url::parse(&url)
            .ok()
            .and_then(|parsed| Authorization::extract_from_url(&parsed));
...
async fn poll_join<T>(pending: &mut Option<JoinHandle<T>>) -> Result<T, JoinError> {
    pending
        .as_mut()
        .expect("guarded by select! `if` clause")
        .await
}
...
async fn send_credential(
    ...
    let _ = events_tx.send(if is_voucher {
        MppEvent::VoucherSent
    } else {
        MppEvent::CredentialSent
    });
    Ok(())
}
...
async fn handle_text<P: PaymentProvider + 'static, V: VoucherProvider>(
    ...
    let server_msg: WsServerMessage = match json_from_str(text) {
        ...
        Err(err) => {
            ...
            let _ = events_tx.send(MppEvent::Error(format!(
                "malformed MPP frame: {err}; not reconnecting"
            )));
            return Err(TerminationReason::Fatal);
        }
    };
